...
const { Option } = Select;
const Credit = ({ setCreditAddView, storeCredit, setStoreCredit, searchData }) => {
  ...
  const handleFinish = (values) => {
    ...
    api.post("credit", credit).then((response) => {
      if (response.status === 201) {
        message.success("Crediter Added Successfully!");
        setStoreCredit([...storeCredit, formattedValues]);
        setCreditAddView(false);
        searchData();
      } else {
        message.error("An error occurred");
      }
      console.log(response, "response from Credit view");
    });
  };
  useEffect(() => {
    if (focusedSelect) {
      ...
      <Form
        ...
        <Form.Item
          ...
          rules={[
            {
              required: true,
              message: "Please select one Crediter",
            },
          ]}>
          <Select
            // mode="multiple"
        ...
        <Form.Item
          ...
          rules={[
            {
              required: true,
              message: "please enter CreditDate",
            },
          ]}>
          <DatePicker
            className="w-full"
        ...
        <Form.Item className="flex">
          ...
          <Button onClick={() => setCreditAddView(false)}>Cancel</Button>
        </Form.Item>
      </Form>
    </div>
  )
